...
runWorker ::
    ...
    -> WorkerState a b
    -> WorkerAction a b c
    -> IO ([Messages], WorkerState a b)
runWorker config state m =
    runStateT (execWriterT $ runReaderT (runApp m) config) state
...
runServer wc ws = do
    ...
    (messages, state') <-
        receiveWait
            [match $ run taskSubmissionHandler, match $ run statusReportHandler]
